...
class DataWrapper:
    ...
    def getAttrDict(self, objFullPath, attrName, startFrame, endFrame, dataType):
        ...
        if keyframeCount != 0 and dataType == 'anim':
            ...
            attrDict['values']['anim']['animKeys'] = self.getKeyframeDict(objFullPath, attrName, keyframeCount,
                                                                          startFrame, endFrame)
            ...
            # These are Bools but works as integers
                ...
                raise StandardError, "# DataContainer.getAttrDict >> Attribute type isn't supported yet. Value in question: " + objFullPath + '.' + attrName + " = " + str(
                    attrValue)
    ...
    def getKeyframeDict(self, objFullPath, attrName, keyframeCount, startFrame, endFrame):
        ...
        attributes = mc.keyTangent(objFullPath + '.' + attrName, time=( startFrame, endFrame ), q=True, inAngle=True,
                                   outAngle=True, inWeight=True, outWeight=True, inTangentType=True,
                                   outTangentType=True, lock=True, weightLock=True)
        frameValues = mc.keyframe(objFullPath + '.' + attrName, time=( startFrame, endFrame ), query=True,
                                  absolute=True, timeChange=True, valueChange=True)
    ...
    def writeToScene(self, selection, selectOrder, namespace, animOffset):
        ...
        if curFps != 0 and srcFps != 0:
            if srcFps != curFps:
                userInput = mc.confirmDialog(title="Framerate doesn't match", message=(
                "Animation was exported with a different framerate than \nyour current settings. Continue anyway? \n\nSource: " + `srcFps` + " fps\nCurrent: " + `curFps` + " fps"),
                                             ma="left", button=["Yes", "No"], defaultButton="No", cancelButton="No",
                                             dismissString="No")
        ...
        if self.dataObj.getHeaderAttr('filetype') == 'anim':
            if isKey:
                proceed = mc.confirmDialog(title="Keys exist", message=(
                "Keys already exist in framerange: " + str(startTime) + '-' + str(stopTime) + '\nOverrwrite?'),
        ...
        if len(rooMismatchDict.values()):
            ...
            for key in rooMismatchDict:
                print key.split("|")[-1] + ": source roo: " + str(
                    rooMismatchDict[key]["sourceRoo"]) + ", target roo: " + str(rooMismatchDict[key]["currentRoo"])
                ...
                if mc.objExists(obj + '.' + attr):
                    ...
                    if self.dataObj.hasAnim(objID, attr):
                        ...
                        for key in self.dataObj.getAttrKeyID(objID, attr):
                            ...
                            mc.setKeyframe(obj, time=frameNr, attribute=str(attr),
                                           value=float(self.dataObj.getKeyAnimData(objID, attr, key, 'value')),
                                           breakdown=False, hierarchy='none', controlPoints=False, shape=False)



                        # Set Tangents
                        mc.keyTangent(obj + '.' + attr, edit=True,
                                      wt=int(self.dataObj.getAttrData(objID, attr, 'weightedTangents')))
                        ...
                        for keyID in self.dataObj.getAttrKeyID(objID, attr):
                            ...
                            try:
                                ...
                                outTangentTypeVal = str(
                                    self.dataObj.getKeyAnimData(objID, attr, keyID, 'outTangentType'))
                                lockVal = int(self.dataObj.getKeyAnimData(objID, attr, keyID, 'lock'))

                                mc.keyTangent(obj + '.' + attr, time=( frameNr, frameNr ), edit=True,
                                              inAngle=inAngleVal, outAngle=outAngleVal, inWeight=inWeightVal,
                                              outWeight=outWeightVal)
                                mc.keyTangent(obj + '.' + attr, time=( frameNr, frameNr ), edit=True,
                                              inTangentType=inTangentTypeVal, outTangentType=outTangentTypeVal)
...
class DataContainer:
    ...
    def hasAnim(self, objID, attr, namespace=None):
        ...
        try:
            ...
            if len(valueKey) == 1:
                ...
                raise StandardError, "# DataContainer.hasAnim >> " + len(
                    valueKey) + " keys in [" + namespace + "][" + objID + "]['objAttrs'][" + attr + "]['values']. Should be 1"
        except KeyError:
    ...
    def getKeyAnimData(self, objID, attr, keyID, keyword):
        try:
            returnVal = \
            self.content['data'][self.defaultNamespace][objID]['objAttrs'][attr]['values']['anim']['animKeys'][keyID][
                keyword]
        except KeyError:
            print "# defaultNamespace: ", self.defaultNamespace
            print "# objID: ", objID
...
class ProgressHandler:
    ...
    def __windowsPrint(self):
        mc.progressWindow(edit=True, progress=int(self.curStatus),
...
class PaieGUI:
    ...
    def makeGUI(self):
        ...
        self.uiPath_topBar = mc.rowLayout(parent=self.uiPath_mainForm, nc=4, adjustableColumn=1,
                                          columnAttach4=("both", "both", "both", "both"),
                                          columnAlign4=("left", "center", "center", "center"))

        mc.radioButtonGrp(columnWidth3=(60, 80, 80), numberOfRadioButtons=2, label="Mode:",
                          labelArray2=("Import", "Export"), select=self.mode, on1=lambda *args: self.setMode(1),
                          on2=lambda *args: self.setMode(2))
        ...
        mc.tabLayout(self.uiPath_tabs, edit=True, childResizable=True,
                     selectTabIndex=self.currentTab + 1)  # +1 when editing tabLayout. Stupid 1-indexing!
        self.updateFilelist()
    ...
    def makeTab(self, index):
        ...
        pathRow = mc.rowLayout(nc=3, adjustableColumn=2, columnAlign=(3, "center"), columnAttach=(1, "left", 5),
                               columnWidth=((1, 80), (3, 60)))

        mc.text(label="Directory:", width=75)
        self.uiPath_pathField[index] = mc.textField(text=self.getCurrentPath(),
                                                    changeCommand=lambda *args: self.setNewPath(*args))
        ...
        if self.mode == 1:  #Import
            ...
            self.uiPath_selectOrder[index] = mc.checkBox(label="Select Order", value=self.selectOrder[index],
                                                         changeCommand=lambda *args: self.setSelectOrder(*args))
            self.uiPath_origNamespaces[index] = mc.checkBox(label="Use Original Namespaces",
                                                            value=self.origNamespaces[index], enable=False,
                                                            changeCommand=lambda *args: self.setOrigNamespace(*args))
            self.uiPath_applyAtOrigin[index] = mc.checkBox(label="Apply At Origin", value=self.applyAtOrigin[index],
                                                           ann="Imports animation onto original frame numers",
                                                           enable=True,
            ...
            self.uiPath_fileList[index] = mc.textScrollList(height=self.listHeight,
                                                            deleteKeyCommand=lambda *args: self.deleteSelectedFile(),
            ...
            self.uiPath_fileInfo[index] = mc.scrollField(height=self.listHeight - 28, editable=False, ww=1,
            ...
            self.uiPath_filename[index] = mc.textFieldGrp(label="Filename", columnAlign=(1, 'left'),
                                                          columnWidth2=(75, 130))
            self.uiPath_exportType[index] = mc.radioButtonGrp(select=self.exportType[index], columnWidth=(1, 100),
                                                              columnAlign=(1, "left"),
                                                              on1=lambda *args: self.setExportType(1),
                                                              on2=lambda *args: self.setExportType(2),
                                                              label="Export Type", labelArray2=["Animation", "Pose"],
                                                              numberOfRadioButtons=2)
            self.uiPath_exportTimeline[index] = mc.checkBox(label="Timeline Only", value=self.exportTimeline[index],
                                                            ann="Exports animation on timeline only (instead of all animation on object)",
                                                            enable=True,
            ...
            self.uiPath_exportButton[index] = mc.button(label="Export", align="center",
                                                        command=lambda *args: self.exportButton())
    ...
    def setNewPath(self, inputPath):
        ...
        if inputPath != None and inputPath != "":
            ...
            if self.currentTab < len(pathArray):
                ...
                self.setTabLabel()
                raise IndexError, "# PaieGUI.setNewPath >> Current tab dosn't match anything. Tab: " + str(
                    self.currentTab)
    ...
    def deleteSelectedFile(self):
        ...
        if proceed == 1:
            ...
            if permissions == 0:
                ...
                answer = mc.confirmDialog(title="Delete File", message="Are you sure you want to delete this file?",
                                          button=["Yes", "No"], defaultButton="Yes", cancelButton="No",
                                          dismissString="No")
                if answer == "Yes":
                    os.remove(filepath)
                    ...
                    curSel = mc.textScrollList(self.uiPath_fileList[self.currentTab], edit=True,
                                               selectIndexedItem=curSel)
                    self.fileSelected()
                    #if self.mode == 0:
    ...
    def browseFolders(self):
        ...
        if platformCase == "ms":
            ...
            mc.fileBrowserDialog(m=4, fc=lambda *args: self.applyBrowsePath(*args), ft="directory", an="Set Work Dir",
                                 om="Import")
            ...
            if self.mode == 1:  # Import
                ...
                if loadVal:
                    ...
                    if len(namespaces) == 1:
                        ...
                        mc.textScrollList(self.uiPath_namespaceList[self.currentTab], edit=True,
                                          selectItem=namespaces[0])
                    else:
                    ...
                    mc.textScrollList(self.uiPath_namespaceList[self.currentTab], edit=True, removeAll=True)
                    mc.scrollField(self.uiPath_fileInfo[self.currentTab], edit=True, clear=True)
                    mc.scrollField(self.uiPath_fileInfo[self.currentTab], edit=True,
    ...
    def importButton(self):
        ...
        if type(selNamespaceList) == type([]) and type(selFileList) == type([]):
            ...
            importData(path + "/" + selFile + self.fileExt, self.selectOrder[self.currentTab], namespace=selNamespace,
...
def __checkFile(filePath):
    ...
    if proceed == 1:
        proceed = mc.confirmDialog(title="File exist", message="File already exist. Overwrite it?",
...
def exportData(filePath, dataType, attrsType='keyable', exportTimeline=1, startFrame=None, endFrame=None, objs=None,
               userInput=None):
    '''
    ...
    try:
        ...
        mc.confirmDialog(title='Error',
                         message="# exportData >> Non-unicode characters aren't supported. Stop using them!",
                         button='OK')
        return 0
